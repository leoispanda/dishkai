...
- Verified Indian Depth Batch 024 with 12 additional high-frequency Indian dishes, bringing Indian coverage to 24. Old depth candidate images were mostly wrong or repeated, so accepted rows received generated replacement images with local metadataCode stamping before entering verified.
...
- Verified database audit Chinese Batch 017 restored/corrected 25 audited Chinese dishes from quarantined generated layers, bringing Chinese coverage to 30 and the verified export to 421 dishes / 1382 aliases. Metadata was rebuilt with Chinese-specific reusable ingredients/seasonings and concrete ordering guidance, including peanut, soy/gluten, egg, shellfish, fish, pork, sesame, chili heat, fermented chili bean paste, black vinegar, and sauce/oil watch-outs. 12 expanded-layer Chinese images were accepted and code-stamped; all 13 global-depth Chinese images were replaced because the candidate sheet showed wrong duplicate images. `hainanese-wonton-noodles` was corrected to `wonton-noodles`, and duplicate `shumai-dim-sum` was replaced by distinct `scallion-pancake` under code 111030.

- Verified database audit World Layer Batches 018-021 restored 120 audited dishes from quarantined `world-dishes.js` in four <=30-dish batches, bringing the verified export to 541 dishes / 1716 aliases and reducing quarantine to 543 rebuild + 26 remove. The batch adds 12-dish verified coverage for Indian, Vietnamese, Indonesian, Malaysian/Singaporean, Turkish, Middle Eastern, Mexican, American, and Moroccan cuisines, plus 12 Greek world-layer classics. Metadata was rebuilt with dish-specific composition/flavor/texture/order guidance, all 120 existing images were human-reviewed via contact sheets and code-stamped, and Turkish/Middle Eastern baklava plus Mexican ceviche aliases were narrowed to avoid cross-cuisine collisions. New reusable global ingredients `tempeh`, `crab`, `lobster`, and `milk-solids` were added for truthful composition instead of using tofu/shrimp/dairy stand-ins.

- Verified database audit Indonesian Batch 027 restored the final 6 Indonesian global-depth dishes (`siomay-bandung`, `martabak-telur`, `martabak-manis`, `klepon`, `es-cendol`, `sambal-goreng-kentang`) after rejecting duplicate/mismatched candidate images and generating replacement-quality code-stamped assets. Indonesian coverage is now 30 dishes; verified export is 643 dishes / 1964 aliases, with 441 rebuild and 26 remove rows still quarantined.

- Added a project-local Codex skill at `skills/dishkai-verified-dish-review/` to preserve the strict verified dish database audit workflow. Future DishKAI dish review/rebuild tasks should update this skill when the review process, quality gates, image workflow, or validation commands change.

- Verified database audit World Truth Batches 022-023 attempted a 200-dish continuation but accepted only 48 rows after content, duplicate, alias, and image review. `cataplana` stayed quarantined because it overlaps the already verified `cataplana-de-marisco`, and `pom` stayed quarantined because the current image reads as roast chicken/potatoes rather than Pom casserole. The accepted rows add Surinamese 11, Ethiopian 12, Scandinavian 1, Peruvian 12, and Brazilian 12 verified dishes, bringing the verified export to 589 dishes / 1834 aliases and quarantine to 495 rebuild + 26 remove. Future 200-scale validation requires replacement-quality images for the rejected global-depth/europe-depth candidates before they can enter verified.
- Corrective note: do not use abstract deterministic schematic/cartoon plate renders as verified dish images. Leo flagged these as too crude to identify dishes at a glance, so the Global Truth Batches 024-033 verified promotions were reverted. Future rebuilds need restaurant-realistic or otherwise clearly recognizable dish images that pass manual visual review, not merely code-matched diagrams.
- 2026-06-20 verified image re-review found `ensaladilla-rusa` had a misleading visual reference that reads closer to a tuna/green-bean composed salad than Spanish mayo potato salad. It was removed from `auditedGeneratedDishIds` and returned to rebuild/replace-image status, reducing the verified export to 588 dishes / 1830 aliases. Do not restore it without a replacement-quality image that clearly shows the creamy potato salad form.
- 2026-06-20 follow-up replaced `ensaladilla-rusa` with a restaurant-realistic creamy potato-mayonnaise salad image showing tuna, egg, peas, olives, and red pepper, then restored it to `auditedGeneratedDishIds`. Spanish is back to 30 verified dishes and the verified export is back to 589 dishes / 1834 aliases. Keep this as the standard for correcting bad verified images: remove first, replace only with a visibly dish-specific image, then restore.
- Indian Depth Batch 025 completed the Indian 30-dish verified target by rebuilding `prawn-curry-indian`, `fish-curry-indian`, `idli`, `vada`, `pakora`, and `kulfi`. The broad bare `prawn curry` and `fish curry` aliases were removed because they can overmatch non-Indian menus; keep seafood curry aliases cuisine-qualified unless a future matcher has menu-context disambiguation. All six rows received restaurant-realistic generated replacement images with local metadataCode stamping. Current verified export is 607 dishes / 1872 aliases; remaining quarantine is 477 rebuild + 26 remove.
- Southeast Asia Depth Batch 026 verified 30 additional global-depth rows: 18 Vietnamese dishes completed Vietnamese coverage at 30, and 12 Indonesian dishes brought Indonesian coverage to 24. Existing quarantined images for these rows were rejected as repeated or semantically wrong, so every accepted row received a restaurant-realistic generated replacement image with local metadataCode stamping. Current verified export is 637 dishes / 1952 aliases; remaining quarantine is 447 rebuild + 26 remove.
- 2026-06-20 text-only verified metadata review audited the 637-dish verified export without changing images. The pass removed internal starter-database/candidate wording from exported base descriptions, replaced café/brunch internal criteria text with diner-facing guidance, tightened allergen/risk alignment for gluten, soy, fish/dashi, sesame, alcohol, pork, egg, dairy, peanut, and shellfish, corrected component modeling for nachos, samosa, mole poblano, duck confit, and loempia, and cleaned mixed English phrases from verified Chinese/Dutch descriptions. Future verified rows should keep riskFlags synchronized with composition allergens and avoid internal audit language in user-facing metadata fields.

- Italian Batch 028 reviewed the optional European depth layer for `101031`-`101060`, rejecting the old mismatched Italian candidate sheet and replacing all 30 images with restaurant-realistic generated assets stamped with exact metadataCode values. The batch rebuilt dish-specific metadata, added reusable Italian ingredients `wild-boar`, `tripe`, `arugula`, and `bresaola`, added safe Italian alias variants for accents/apostrophes/specific menu names, and brought Italian verified coverage to 60 dishes / full verified export to 673 dishes / 2035 aliases. Future European depth work should remain optional and batch-limited rather than restoring all 60-row Europe layers wholesale.

- French Batch 029 reviewed the optional European depth layer for `103031`-`103060`, rejecting the old mismatched French candidate sheet and replacing all 30 images with restaurant-realistic generated assets stamped with exact metadataCode values. The batch rebuilt dish-specific metadata, added reusable French ingredients for Alpine cheese, frisée, galette/socca bases, charcuterie, crème anglaise, puff pastry, lemon curd, orange caramel sauce, pike quenelle, duck breast, and entrecôte, added safe accent/menu-name aliases, and brought French verified coverage to 60 dishes / full verified export to 703 dishes / 2120 aliases. Remaining European depth rows stay quarantined until strict <=30-dish review batches.

- Malaysian/Singaporean Batch 030 completed the 30-dish target by rebuilding `118013`-`118030` after rejecting the old mismatched candidate sheet. All 18 accepted rows received restaurant-realistic generated replacement images stamped with exact metadataCode values, dish-specific metadata, reusable Malaysian/Singaporean ingredients/seasonings, and safe alias variants. Malaysian/Singaporean verified coverage is now 30 dishes; full verified export is 721 dishes / 2182 aliases. Continue with other below-30 world cuisines before optional European depth.

- World Depth Batches 031-033 restored 90 verified dishes across Turkish, Middle Eastern, Mexican, Moroccan, and Ethiopian after rejecting the existing mismatched/shifted candidate images and generating restaurant-realistic replacement assets with visible metadataCode stamps. `makloubeh` stayed out as a duplicate transliteration of `maqluba`, and `kunafa` was added as the independent Middle Eastern replacement under `120031`. Turkish, Middle Eastern, Mexican, Moroccan, and Ethiopian are now each at 30 verified dishes; full verified export is 811 dishes / 2423 aliases. Remaining below-30 cuisines are American, Surinamese, Peruvian, and Brazilian.

- World Depth Batches 034-037 restored 120 verified dishes across American, Surinamese, Peruvian, Brazilian, British-Irish, and Portuguese. American, Surinamese, Peruvian, and Brazilian are now at 30 verified dishes; British-Irish is 59 and Portuguese is 48 through optional reviewed depth. Eight existing American cafe/brunch images were accepted, 112 rows received restaurant-realistic code-stamped replacement images, `scrambled-eggs` remains quarantined, `minted-peas` is modeled as a side, and broad aliases such as bare `cheesecake`, `cha siu`, `kip sate`, `tapioca`, and `alfajores` were narrowed. Full verified export is now 931 dishes / 2713 aliases.

- Remaining European Depth Batches 039-044 restored 158 additional verified dishes after rejecting mismatched candidate sheets and regenerating restaurant-realistic code-stamped images. Dutch, Spanish, German, and Scandinavian now have 60 verified dishes; Belgian is 56, Portuguese is 59, Greek is 45, and the full verified export is 1089 dishes / 3042 aliases. Duplicate/unsafe rows such as `cataplana`, `makloubeh`, `beef-tartare-belgian`, `grey-shrimp-tomato`, and `americain-prepare` stayed quarantined or had narrow aliases remapped to canonical verified dishes. `scripts/import-generated-dish-image.py` is now the safe local helper for copying generated PNGs into batch archives and producing stamped main/thumb WebP assets.

- Added a special unmatched-card path for `chef special pasta` / chef-special-style menu items. It stays `metadataSource: unmatched` and `verified: false`, uses a generic "Chef Special / Ask the staff" visual asset, and tells diners to ask the server about today's sauce, main ingredients, allergens, and portion instead of pretending the dish has a fixed verified recipe.

- Real Menu Miss Batch 045 reviewed the centralized `DISHKAI_UNMATCHED_DISHES` KV backlog snapshot (14 events, 123 raw sightings, 98 unique not-yet-verified items) and promoted only 12 stable, reusable restaurant dishes: `pad-prik-kaeng`, `pad-pong-karee`, `teriyaki-salmon`, `mojama`, `pan-de-cristal`, `ravioles-gratin`, `onglet-de-boeuf`, `andouillette`, `tete-de-veau`, `truite-grenobloise`, `truite-aux-amandes`, and `moelleux-au-chocolat`. It also added safe aliases for already verified `cilbir`, `bbq-ribs`, `sticky-toffee-pudding`, and `jamon-iberico`. Generic proteins, sides, menu buckets, and restaurant-specific specials stayed unverified/fallback-only. Verified export is now 1101 dishes / 3116 aliases.
- Compound Menu Miss Batch 046 promoted 9 stable compound/assembled dishes from the remaining real-menu miss backlog: `schiacciata-mortadella-stracciatella`, `oeuf-en-cocotte`, `poulet-aux-ecrevisses`, `poulet-aux-morilles`, `aceitunas-rellenas-de-anchoas`, `sardinas-en-aceite`, `tandoori-chicken-naan`, `blackened-eggplant-tahini`, and `poke-bowl`. The batch added reusable ingredients for crayfish, morel mushrooms, naan bread, mortadella, and stracciatella cheese, plus risk inference for mortadella/naan. Future compound backlog reviews should abstract to stable dish boundaries and keep one-off garnish stacks or generic sides out of verified. Verified export is now 1110 dishes / 3160 aliases.
- DishKAI v0.2.28 added a universal generic ordering-card layer for restaurant-common broad terms: fries, salad, steak/ribeye, grilled chicken, prawns/shrimp, dessert, and drinks. These cards reduce avoidable unmatched results and KV backlog noise but remain `matchStatus: universal`, `metadataSource: universal-generic`, and `verified: false`; they are guidance cards, not verified dish metadata.
- Real Menu Miss Batch 047 promoted 8 stable Greek/French backlog items with restaurant-realistic code-stamped images: `bekri-mezes`, `arnaki-tigania`, `fromage-blanc-dessert`, `religieuse`, `tarte-amandine`, `cafe-gourmand`, `chocolat-liegeois`, and `cafe-liegeois`. It also added `cheese platter` / `cheese plate` / `assiette de fromages` aliases to existing `plateau-de-fromages`. Generic proteins, drinks, and one-off dessert names stayed out of verified. Verified export is now 1118 dishes / 3219 aliases.
- Real Menu Miss Batch 048 promoted `almendras-fritas` and `murcon-aux-ravioles` with restaurant-realistic code-stamped images, and remapped explicit Belgian tartare aliases to existing `filet-americain` instead of duplicating `beef-tartare-belgian`. Bokit, Accra, and Coconut pie should remain deferred until a Caribbean/French-Caribbean cuisine taxonomy decision is made. Verified export is now 1120 dishes / 3238 aliases.
- General Batch 001 created the verified `general` cuisine for stable cross-restaurant dishes that are more specific than universal fallback cards. It added `coleslaw`, `corn-on-the-cob`, `garlic-bread`, `onion-rings`, `mashed-potatoes`, `mixed-vegetables`, `grilled-vegetables`, and `side-rice` under metadata codes `100001`-`100008` with restaurant-realistic code-stamped images. Keep `general` for specific verified menu items only; broad buckets such as steak, salad, dessert, drink, lamb chops, and sole fillet should stay universal/context-aware unless later reviewed as stable dish records. Verified export is now 1128 dishes / 3296 aliases across 28 cuisines.
- 2026-06-23 live `DISHKAI_UNMATCHED_DISHES` KV check found 18 upload events, 142 unique unknown/AI-generated menu items, and 175 total sightings stored online; latest event was 2026-06-23T10:22:52Z. This is historical raw storage and may include items already solved by later metadata batches until the KV backlog is filtered or cleared.
- General Batch 002 extended verified `general` coverage with `potato-wedges`, `roasted-potatoes`, `baked-potato`, `tomato-soup`, `cream-of-mushroom-soup`, `mozzarella-sticks`, `garlic-mushrooms`, and `baked-beans` under metadata codes `100009`-`100016`. The batch added restaurant-realistic code-stamped images and narrow aliases, including `gebonden tomatensoep` and `gebonden champignonsoep` from the live KV signal, and removed the old overbroad `roasted potatoes` / `geroosterde aardappels` alias from `greek-restaurant-sides`. Verified export is now 1136 dishes / 3357 aliases across 28 cuisines.
- AI Generated Backlog Batch 049 reviewed the remaining AI-generated backlog signal and promoted only stable reusable items: Spanish `chorizo-tapas` plus general `smoothie-bowl`, `ice-cream-sundae`, `fruit-sorbet-plate`, `applesauce-side`, `chicken-skewer`, `chicken-fillet-mushroom-sauce`, `lamb-chops`, and `sole-fillet`. Broad items such as `cheesecake`, `beef tenderloin`, `pork tenderloin`, `lamb fillet`, and `white chocolate dessert` should stay universal unless later reviewed as specific dish records. Verified export is now 1145 dishes / 3444 aliases across 28 cuisines.
- General Batch 003 promoted 10 additional stable cross-restaurant dishes into verified `general`: `waffles`, `grilled-salmon`, `mixed-grill`, `tuna-salad`, `chicken-wings`, `roast-chicken`, `side-salad`, `fruit-salad`, `cheesecake`, and `chocolate-cake` under metadata codes `100025`-`100034`. Exact `grilled salmon` / `zalm` / `solomos` and `mixed grill` now belong to general unless a cuisine-specific verified card exists; universal keeps only broad buckets such as `salad`, `dessert`, `cake`, and `grilled chicken`. Verified export is now 1155 dishes / 3531 aliases across 28 cuisines.
- DishKAI v0.2.29 simplified the public UI into an orange Apple/Revolut-inspired frosted-glass experience. Keep the homepage action-first with short copy, collapsed legal details, Text/Photo + Language + Analyze as the primary controls, compact ordered results first, and full dish knowledge only after selection.
- DishKAI v0.2.30 unified the footer into a single orange-glass information bar. Keep footer items on the same visual system: compact brand/version, one short product line, contact pill, and disclaimer pill with matching height, border, background, weight, and alignment.
- DishKAI v0.2.31 removed the footer version pill. Keep the version as quiet small text near the brand, without border, background, shadow, or repeated `DishKAI` wording.
- DishKAI v0.2.32 changed the footer slogan to `Know the dish with AI before you order.` The main hero can stay ordering-confidence-first, but the footer may use short AI positioning. Keep the right-side footer action pills visually identical in color, border, background, hover weight, and icon treatment.
- DishKAI v0.2.33 restored small dish thumbnails in the mobile menu list. Keep the compact ordered list first, but do not hide verified thumbnails on narrow screens; use a reduced fixed thumb column so rows stay scannable.
- DishKAI v0.2.34 changed dish detail opening from a separate lower Knowledge Card section to inline expansion inside the ordered menu list. Keep the original menu order visible and expand details in place so mobile users do not lose scroll context.
- DishKAI v0.2.35 added a verified-alias safety gate: runtime aliases are normalized, deduplicated, checked for cross-dish conflicts, and resolved only through an explicit ownership policy; broad translations that cannot safely identify one dish stay quarantined. Partial matching now prefers the longest safe alias, and `scripts/check-menu-matching.mjs` protects representative conflict and prefix cases.
- DishKAI v0.2.35 also keeps private PDC initialization off the normal public path, clears stored PDC state after confirmed session expiry/logout, bounds menu-image request streams before multipart parsing, moves unmatched-dish KV recording behind Pages `waitUntil`, and keeps the PDC entry visible only after successful private authentication.
