...
- Strict Global Truth Batches 029-033 restored 90 verified dishes across Mexican, Moroccan, Surinamese, Peruvian, Brazilian, and one Middle Eastern dish after rejecting the old shifted global-depth images in contact-sheet review. Verified export is now 769 dishes / 2280 aliases; high-risk quarantine is 315 rebuild + 26 remove. Generic aliases such as generic arroz con leche, ajiaco, jalea, kibe, and tapioca were narrowed to cuisine-specific forms to avoid cross-cuisine false matches.
...
- Verified database audit Global Truth Batches 024-028 restored exactly 90 audited `global-depth-dishes.js` rows in five <=18-dish batches: Indian +18, Vietnamese +18, Indonesian +18, Malaysian/Singaporean +18, and Turkish +18. The old candidate images were rejected because the contact sheets showed systemic shifted/repeated dishes, so these batches use deterministic semantic visual-reference renders with exact visible metadata codes and matching thumbs rather than pretending the wrong photos were valid. Metadata was rebuilt with dish-specific composition/flavor/texture/risk/order guidance, generic aliases such as `prawn curry`, `fish curry`, and `Beef Rendang` were narrowed, and reusable ingredients/seasonings were added for paneer, rice vermicelli, lontong, fish paste, bulgur, sesame, shrimp paste, keluak, and rosewater. Verified export is now 679 dishes / 2060 aliases; quarantine is 405 rebuild + 26 remove.
